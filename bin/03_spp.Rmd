...
read <- read_tsv((params$readsfilt), col_types = "cci") %>%
  filter(filtering == "2k") %>%
...
samples_all <- import_sample_names((params$samplenames))
...
if (dim(staph)[1] == 0){
  ...
  if (file.exists(params$vfdb) && file.size((params$vfdb)) == 0){
    ...
    df <- read_tsv((params$vfdb), col_types = "c____c___dd_____", col_names = c("Isolate", "Gene", "Percent identity (%)", "Percent coverage (%)")) %>%
    select(Isolate, Gene, `Percent identity (%)`, `Percent coverage (%)`) %>%
    mutate(Isolate = str_remove_all(Isolate, "_flye.fasta|_assembly.fasta|.*/")) %>%
...
if (dim(top)[1] == 0){
  ...
  if (dim(subset(top, !(Isolate %in% failed_samples)))[1] != 0){
    ...
    df <- read_tsv((params$emm), col_types = "c__c___c", col_names = c("Isolate", "type", "cluster")) %>%
    mutate(Isolate = str_remove_all(Isolate, "_flye.tmp|_assembly.tmp")) %>%
    arrange(Isolate) %>%
