...
- The [venae pipeline](https://github.com/phac-nml/venae) v0.2.1 managed by [Nextflow](https://www.nextflow.io/) was run to analyze the ONT sequencing reads and output this report
...
- Assemblies were generated with [metaFlye v2.9.4](https://github.com/fenderglass/Flye) using reads that were >1 kb in length
...
- Antimicrobial resistance gene detection was performed using the following tools and required a minimum > 90% identity to the reference gene:
  ...
  - [kmerResistance v2.2.0](https://bitbucket.org/genomicepidemiology/kmerresistance/src/master/) using the ResFinder database v2.4.0 (2024-08-06)  
  - [ChroQueTas v1.0.0](https://github.com/nmquijada/ChroQueTas) using the FungAMR database v1.0.0 (June 2025)
...
- Predicted resistance phenotypes for fungi are derived from the FungAMR database and does not represent all resistant determinants or point mutations
    - These phenotypes have not been validated and may not represent the true phenotype for each antimicrobial
    - Predicted resistance should be confirmed by phenotypic susceptibility testing
- Predicted resistance phenotypes for fungi in the Summary Table \@ref(tab:summary) only show those antimicrobials with resistance confidence scores between 1 and 4
    - For more information on confidence scores, see [here](https://github.com/nmquijada/ChroQueTas/wiki/Confidence-score-for-antimicrobial-resistance)
- Predicted resistance phenotypes for bacteria are derived from the Centre for Genomic Epidemiology ResFinder database does not represent all resistant determinants or point mutations
...
- Predicted resistance phenotypes for bacteria in the Summary Table \@ref(tab:summary) only show those antimicrobials included in the Clinical and Laboratory Standards Institute document [Performance Standards for Antimicrobial Susceptibility Testing: Informational Supplement M100 ED35:2025](https://clsi.org/standards/products/microbiology/documents/m100/) Table 1 (Tier 1 and Tier 2)
...
for (i in 1:length(clsi_files)) {
    temp <- read_tsv((clsi_files[i]), col_types = "c____", col_names = c("antibiotics")) %>%
      mutate(antibiotics = str_replace(antibiotics, "-", "+")) %>%
      mutate(antibiotics = str_to_title(antibiotics)) %>%
      mutate(table = str_remove_all(clsi_files[i], "_.*")) %>%
      # mutate(table = str_to_title(table)) %>%
      mutate(organism = str_remove_all(clsi_files[i], "Table1[^_]*_|.tsv")) %>%
