...
import_sylph_tax <- function(x) {
  read_tsv(x, col_names = c("sample", "clade_name", "taxonomic_abundance", "sequence_abundance", "ani", "coverage"), col_types = "ccdddd") %>%
  ...
  mutate(sample = str_remove_all(sample, "spp_sylph/|_cleanfilt2k.fastq.gz.sylphmpa|.sylphmpa"),
...
import_read_metrics <- function(x) {
  read_tsv(x, col_types = "ccd", col_names = c("sample", "metric", "value")) %>%
