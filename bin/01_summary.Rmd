...
clsikey <- read_tsv((params$clsikey), col_types = "cccc")
...
for (i in 1:length(clsi_files)) {
    clsi[[i]] <- read_tsv((clsi_files[i]), col_types = "c____", col_names = c("antibiotics")) %>%
      mutate(antibiotics = str_replace(antibiotics, "-", "+"))
    names(clsi)[i] <- str_remove_all(clsi_files[i], "Table1[^_]*_|.tsv")
}
...
if(!file.exists(params$chroquetas) || file.size((params$chroquetas)) == 0){
  ...
  df3 <- tibble(sample = character(), fungamr_phenotype = character())

} else {

  df3 <- import_chroquetas(params$chroquetas) %>%
    filter(resistance_confidence == "1" | resistance_confidence == "2" | resistance_confidence == "3" | resistance_confidence == "4" | protein == "None") %>%
    group_by(sample) %>%
    summarise(fungamr_phenotype=paste(phenotype, collapse=",")) %>%
    mutate(fungamr_phenotype = ifelse(fungamr_phenotype == "NA", "None detected", fungamr_phenotype))

  df3$fungamr_phenotype <- sapply(strsplit(df3$fungamr_phenotype, ",", fixed = TRUE), function(x) paste(unique(sort(x)), collapse = ", "))

}

# join ChroQueTas to sample info/all and concat phenotypes
all2 <- left_join(all, df3, by = "sample") %>%
  mutate(phenotype = case_when(
    is.na(cge_phenotype) & is.na(fungamr_phenotype) ~ NA,
    is.na(cge_phenotype) & fungamr_phenotype == "None detected" ~ "None detected",
    cge_phenotype == "None detected" & is.na(fungamr_phenotype) ~ "None detected",
    cge_phenotype == "None detected" & fungamr_phenotype == "None detected" ~ "None detected",
    TRUE ~ paste(cge_phenotype, fungamr_phenotype, sep = ", "))
    ) %>%
  mutate(phenotype = str_remove_all(phenotype, ", NA|NA, |, None detected|None detected, ")) %>%
  select(!c("cge_phenotype", "fungamr_phenotype"))
all2$phenotype <- sapply(strsplit(all2$phenotype, ", ", fixed = TRUE), function(x) paste(unique(sort(x)), collapse = ", "))

# import read metrics
read <- import_read_metrics((params$reads))

# assign qc pass/fail based on species ID, median read length, median read quality
all_qc <- left_join(all2, read, by = "sample") %>%
  ...
  select(Sample = sample, Organism = organism, `Polymicrobial` = poly, `QC` = qc, `Predicted resistance phenotype` = phenotype) %>%
  mutate(`QC` = ifelse(`QC` == "fail", cell_spec(`QC`, "html", color = "#F8766D", bold = TRUE), cell_spec(`QC`, "html", color = "#00BFC4", bold = TRUE))) %>%
  mutate(`Predicted resistance phenotype` = ifelse(`Predicted resistance phenotype` == "NA" | is.na(`Predicted resistance phenotype`) | `Predicted resistance phenotype` == "", "Could not predict", `Predicted resistance phenotype`))
...
kable(all_qc, caption="Summary results", digits = 2, align = "l", "html", escape = "F") %>%
  ...
  footnote(symbol = c(
    ...
    "Predicted phenotype is derived from the Centre for Genomic Epidemiology ResFinder (bacteria) and FungAMR (fungi) databases and does not represent all resistant determinants or point mutations.
      **These phenotypes have not been completely validated in all organisms and may not represent the true phenotype for each antimicrobial.**
      Predicted antibacterial resistance phenotypes only show those antimicrobials included in the Clinical and Laboratory Standards Institute document Performance Standards for Antimicrobial Susceptibility Testing: Informational Supplement M100 ED35:2025 Table 1 (Tier 1 and Tier 2).
      Please see Table \\@ref(tab:clsitable) for the full list of reportable antibiotics in Tier 1 and Tier 2 for each organism.
      Predicted antifungal resistance phenotypes only show antifungals with strong evidence (confidence scores between 1-4).
      **Predicted antifungal resistance should be confirmed by phenotypic susceptibility testing.**
      \"Could not predict\" indicates AMR prediction is not supported for this organism or that there was not enough sequencing data to generate an assembly.\n"))
